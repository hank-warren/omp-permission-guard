...
function extractCommandSubstitution(
	...
	while (i < tokens.length && depth > 0) {
		...
		if (depth === 1 && _isAttachedCommandSubstitutionStart(tokens, i)) {
			...
			const { innerSegments: nestedSegments, endIndex } = extractCommandSubstitution(tokens as ParseEntry[], i + 2);
			for (const seg of nestedSegments) {
				innerSegments.push(seg);
			}
			i = endIndex + 1;
...
function _stripAttachedIoNumbers(command: string): string {
	...
	for (let i = 0; i < command.length; ) {
		...
		if (!inSingle && char === "`") {
			...
			if (atTokenBoundary && command[endIndex + 1] && _isPathLikeBacktickSuffix(command[endIndex + 1] as string)) {
...
export function stripWrappersWithInfo(tokens: string[], cwd?: string | null): WrapperStrippingResult {
	let result = [...tokens];
...
function stripSudoWithInfo(tokens: string[], cwd?: string | null): { tokens: string[]; cwd?: string | null } {
	let i = 1;
	let currentCwd = cwd;
	while (i < tokens.length) {
		const token = tokens[i];
...
const ENV_OPTS_WITH_VALUE = new Set(["-u", "--unset", "-C", "--chdir", "-S", "--split-string", "-P"]);
...
function stripEnvWithInfo(tokens: string[], cwd?: string | null): EnvStrippingResult {
	...
	while (i < expandedTokens.length) {
		...
		if (token === "-S" || token === "--split-string") {
			...
			expandedTokens = [...expandedTokens.slice(0, i), ...splitTokens, ...expandedTokens.slice(i + 2)];
			continue;
		}
		...
		if (token.startsWith("-S") && token.length > 2) {
			...
			expandedTokens = [...expandedTokens.slice(0, i), ...splitTokens, ...expandedTokens.slice(i + 1)];
			continue;
		}
...
function isOperator(token: ParseEntry): boolean {
	return typeof token === "object" && token !== null && "op" in token && SHELL_OPERATORS.has(token.op as string);
}
...
function _isRedirectOp(token: ParseEntry | undefined): boolean {
	return typeof token === "object" && token !== null && "op" in token && REDIRECT_OPS.has(token.op as string);
}
...
function _isAttachedCommandSubstitutionStart(tokens: readonly ParseEntry[], index: number): boolean {
