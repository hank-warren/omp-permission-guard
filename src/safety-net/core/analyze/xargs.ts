...
const REASON_XARGS_RM = "xargs rm -rf with dynamic input is dangerous. Use explicit file list instead.";
...
export function analyzeXargs(tokens: readonly string[], context: XargsAnalyzeContext): string | null {
	...
	if (childCommand.head === "git") {
		...
		const hasDynamicReplacement =
			...
			(childTokens.some(token => token.includes(replacementToken)) ||
				Array.from(childCommand.envAssignments.values()).some(value => value.includes(replacementToken)));
