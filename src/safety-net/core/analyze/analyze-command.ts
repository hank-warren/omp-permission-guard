...
const REASON_STRICT_UNPARSEABLE = "Command could not be safely analyzed (strict mode). Verify manually.";

export const REASON_RECURSION_LIMIT = "Command exceeds maximum recursion depth and cannot be safely analyzed.";
...
export function analyzeCommandInternal(command: string, depth: number, options: InternalOptions): AnalyzeResult | null {
		...
		const reason = analyzeSegment(segment, depth, {
			...
			analyzeNested: (nestedCommand: string, overrides?: AnalyzeNestedOverrides): string | null => {
				...
				const nestedEffectiveCwd =
					overrides && Object.hasOwn(overrides, "effectiveCwd") ? overrides.effectiveCwd : effectiveCwd;
				return (
...
export function applyShellGitContextEnvSegment(tokens: readonly string[], state: ShellGitContextEnvState): void {
...
function isTrackedGitEnvName(name: string): boolean {
	return (
		GIT_CONTEXT_ENV_OVERRIDE_NAMES.has(name) || GIT_CONFIG_AFFECTING_ENV_NAMES.has(name) || isGitConfigEnvName(name)
	);
}
...
function getInitiallyExportedGitContextNames(effectiveEnvAssignments?: ReadonlyMap<string, string>): Set<string> {
...
function setShellGitContextAssignment(state: ShellGitContextEnvState, assignment: GitContextAssignment): void {
...
function setEffectiveGitContextAssignment(state: ShellGitContextEnvState, assignment: GitContextAssignment): void {
...
function getTypesetOperandsInfo(tokens: readonly string[], commandIndex: number): TypesetOperandsInfo | null {
	let i = commandIndex + 1;
