...
export const REASON_INTERPRETER_DANGEROUS = "Detected potentially dangerous command in interpreter code.";
...
export function analyzeSegment(tokens: string[], depth: number, options: InternalOptions): string | null {
	if (tokens.length === 0) {
		return null;
	}
...
const CWD_CHANGE_REGEX = /^\s*(?:\$\(\s*)?[({]*\s*(?:command\s+|builtin\s+)?(?:cd|pushd|popd)(?:\s|$)/;
