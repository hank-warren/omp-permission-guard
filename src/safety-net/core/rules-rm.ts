...
const REASON_RM_RF_ROOT_HOME = "rm -rf targeting root or home directory is extremely dangerous and always blocked.";
const REASON_RM_HOME_CWD = "rm -rf in home directory is dangerous. Change to a project directory first.";
...
export function analyzeRm(tokens: string[], options: AnalyzeRmOptions = {}): string | null {
	const { cwd, originalCwd, paranoid = false, allowTmpdirVar = true, tmpdirOverridden = false } = options;
...
function reasonForClassification(classification: TargetClassification, ctx: RmContext): string | null {
