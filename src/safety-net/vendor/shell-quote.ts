/**
 * Vendored from shell-quote@1.10.0 — parse.js only (the sole function the
 * command analyzer needs). MIT, (c) 2013 James Halliday; see ./shell-quote.LICENSE.
 *
 * Inlined as source so the plugin has ZERO external runtime dependencies: omp
 * marketplace installs symlink the plugin without running `bun install`, so a
 * declared npm dep would be missing at load. Logic is unchanged from upstream.
 */
...
export type Env = Record<string, string | undefined> | ((key: string) => string | object | undefined);
